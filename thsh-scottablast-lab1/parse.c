...
int parse_line (char *inbuf, size_t length,
  ...
  //inbuf is pointer to input
  //create a ptr to loop through inbuf.
  //loop through until you reach non whitespace. At that point, create a pointer named
  //startptr.
  //continue to loop until you find whitespace, a # or \n. that is your command to
  //add.
  //continue to loop through until you come across a # or \n. at that point, end
  //the line by adding \0.
  ...
  int pipeIndex = 0;
  int result = 0;
  while(*ptr != 10 && *ptr != '#') { //while we haven't reached /n
    if(*ptr == '<') { //infile
	    if(started) {
			commands[pipeIndex][argsIndex] = strndup(startptr, ptr-startptr);
			result++;
			argsIndex++;
			started = false;
		}
		//loop until you find another character
		ptr++;
		while(*ptr != 10 && *ptr != '#' && isspace(*ptr) != 0 && *ptr != '|') {
		ptr++; }
		//set that character as startptr
		startptr = ptr;
		//loop until you reach whitespace or /n / #
		while(*ptr != 10 && *ptr != '#' && isspace(*ptr) == 0 && *ptr != '|') { 
	    ptr++;}
		//set that word to either input or output.
		*infile = strndup(startptr, ptr-startptr);
		//continue as normal. NO ptr++
	}
	else if (*ptr == '>') { //outfile
		if(started) {
			commands[pipeIndex][argsIndex] = strndup(startptr, ptr-startptr);
	    ...
	    ptr++;}
		//set that word to either input or output.
		*outfile = strndup(startptr, ptr-startptr);
		//continue as normal. NO ptr++    
	}
	
	else if(*ptr == '|') { //if pipe detected
		//if a command is started,
		//  add it. 
		//	reset started = false
		if(started) {
			commands[pipeIndex][argsIndex] = strndup(startptr, ptr-startptr);
		...
		commands[pipeIndex][argsIndex] = NULL;
		argsIndex = 0; 
		pipeIndex++;
		ptr++;
		//  add line ender \0
		//  reset argsIndex, increment pipeIndex
	}
	else if(!started) {
		if(isspace(*ptr) == 0) { //if we have found a character
			startptr = ptr;
			started = true;
		}
		ptr++;
		...
		if(isspace(*ptr) != 0) { //we have found whitespace
			commands[pipeIndex][argsIndex] = strndup(startptr, ptr-startptr);
			result++;
			argsIndex++;
			started = false;
  ...
  //endcase if started == true and we exited because \n or #
  if(started) {
			commands[pipeIndex][argsIndex] = strndup(startptr, ptr-startptr);
			result++;
			commands[pipeIndex][argsIndex + 1] = NULL;
  }
    argsIndex = 0;
	pipeIndex++;
    commands[pipeIndex][argsIndex] = NULL;  
